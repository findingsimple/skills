...
- **Sandbox-safe commands** — consolidate API calls and data processing into permanent Python scripts. Cross-skill API clients live in `_lib/` (imported via `_libpath.py`); skill-specific processing scripts live in the skill's own dir. Use `urllib.request` inside Python instead of curl to avoid sandbox approval prompts. Never use the Write/Edit tool for `/tmp/` files or files outside the working directory (e.g., Obsidian vault paths) — use `cat << 'SKILL_EOF' > path` via Bash for new file creation. For **modifying** an existing vault file post-creation (replacing a placeholder section, appending to a footer, etc.), use a Python heredoc (`python3 << 'PYEOF' ... PYEOF`) that does an `assert ... in text; text.replace; tmp + os.replace` atomic in-place rewrite — Edit requires a prior Read, and Read on iCloud-synced vault paths can trigger TCC prompts or be blocked by the sandbox. Reference: `reflect/SKILL.md` Step 6 — the canonical pattern for atomic vault-file modification. This also avoids macOS TCC prompts for iCloud paths in scheduled tasks. Always use `SKILL_EOF` as the heredoc delimiter (not `EOF`) to avoid collision with markdown content. Never use inline `python3 -c` with complex quoting (triggers "obfuscation" warnings). Spawned agents (via the Agent tool) cannot use the Read tool on `/tmp/` paths — instruct them to use `cat` via the Bash tool instead.
...
- **Orchestrated pipelines need a re-render escape hatch** — multi-stage skills behind a session lock (setup → fetch → analyze → bundle → agents → apply → report) should give the report stage a `--from-cache` (or `--render-only`) flag that skips `verify_session()` and `clear_stale_state()` when `setup.json` + `analysis.json` are both present and the user explicitly opts in. Otherwise "re-run the report" — which the user will absolutely ask for after tweaking the renderer — degrades into manually crafting a lock file with the old session UUID, or eating a full re-fetch (including expensive sub-agent calls) the user didn't want. Don't make `--from-cache` the default; the lock exists for concurrent-run safety. The renderer is just the cheap, idempotent, frequently-iterated stage where the cost calculus inverts. Reference: `support-trends` is the canonical lock-protected pipeline that currently lacks this; add it next time that file is touched.
...
- **Test explicit-null at every external API boundary** — when writing tests for client code that consumes external API responses (Jira, GitLab, Confluence, Bonusly, etc.), include a case where every optional nested field is **explicitly `None`**, not just missing. `dict.get(key, default)` returns the default only when the key is **absent** — if the key is present with value `None`, `.get()` returns `None` and the next `.foo()` or `.extend()` call crashes in production. This is the most common shape of NPE in HTTP-client glue code, and it never trips in dev because dev responses are usually well-formed. Prefer the `or {}` / `or []` idiom over `dict.get(key, default)` for fields where the API may return null. Reference: `_lib/test_gitlab_client.py::test_handles_author_explicitly_none` and `_lib/test_bonusly_client.py::test_result_null_does_not_crash` — both pin real production bugs caught only after tests were added.
...
- **Wiki links only for vault-resident entities** — only emit `[[X]]` when X actually exists as a `.md` file in the user's vault (people, teams, retros, incidents, root causes). For external references (Jira ticket keys, GitLab MR IIDs, Confluence pages outside the synced vault, arbitrary URLs), emit a markdown hyperlink `[label](url)` instead. A wiki link to a non-existent file renders as a broken/dangling link in Obsidian — visual noise that buries the working links. Reference: `support-trends/report.py:_key_link()` renders `[ECS-1234](https://.../browse/ECS-1234)` rather than `[[ECS-1234]]` because tickets aren't vault files. Sweep the same anti-pattern in any renderer that catalogs Jira keys (root-cause-suggest, root-cause-triage, sprint-summary, support-routing-audit) when next touched.
