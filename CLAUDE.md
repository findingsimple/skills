...
- **Graph-invisible links for index pages** — index/catalog pages (flat lists of all incidents, triage issues, plans) should use standard markdown links `[text](path.md)` instead of wiki links `[[page]]`. Standard markdown links are clickable in Obsidian for navigation but invisible to the graph view. This prevents index pages from becoming mega-hub nodes that dominate the graph. Reserve `[[wiki links]]` for pages that encode meaningful relationships (cross-references, team membership, participation). Rule of thumb: if a page lists items just because they share a type (e.g., "all incidents"), use `[text](path.md)`; if it links items because of a semantic relationship (e.g., "team members", "sprint timeline"), use `[[wiki links]]`.
