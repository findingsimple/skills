...
- **Frontmatter tags for graph clustering** — skills that generate incident or triage files should emit `tags:` in YAML frontmatter to create natural graph clusters via Obsidian's tag nodes (`showTags: true` in `graph.json`). Incidents use `tags: [incident]` plus `sev/{level}` for non-default severity and any `team-*` labels. Triage issues use `tags: [root-cause]` plus classification tag (e.g., `code-bug`, `feature-request`) added during enrichment. Keep tags stable and categorical — avoid per-issue or high-cardinality tags.
