...
Scan existing Obsidian vault files and add `[[wiki links]]` to known entities (people, incidents, triage issues). Generates sprint timeline index pages. Idempotent — safe to re-run.
...
> **Note:** `Incidents/_Index.md` is generated by `/incident-kb` (which has access to severity data), not by vault-linker.
