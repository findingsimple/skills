...
def cmd_prepare(args):
    ...
    skipped_score = 0
    skipped_existing = 0
    existing_keys = existing_result_keys(AUTOFILL_DIR)
    ...
    for i in range(0, len(blocks), batch_size):
        ...
        prompt_path = os.path.join(AUTOFILL_DIR, "batch_%d.txt" % batch_num)
        output_path = os.path.join(AUTOFILL_DIR, "results_batch_%d.json" % batch_num)
    ...
    manifest = {"type": "autofill", "batch_count": len(batches), "batches": batches}
...
def cmd_apply(args):
    ...
    results = load_results(AUTOFILL_DIR)
    if not results:
        print("No autofill results found in %s/" % AUTOFILL_DIR)
        print("Run the agent autofill step first.")
        sys.exit(1)

    # Materialize per-key cache files for any downstream consumers / re-runs
    if not args.dry_run:
        materialize_per_key_cache(AUTOFILL_DIR, results)
